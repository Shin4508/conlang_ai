...
# generate 100 each and select farest one and put it in prompt then loop it
# sematic mapping

new_library = []
def generate_loop(input_text, temperature = 0.5, max_len = 11):
  model.eval()
  new_line_count = 0
  input_ids = []
  for _ in input_text:
  ...
  # make it to a list
  new_words = next_word.split("\n")

  #make the list of the ipa that used in generate iteration
  used_ipa = sorted(list(set(next_word)))
  used_ipa.remove("\n")
  # find the least appeared ipa
  # choose one that includes least ipa
  ipa_count = 0
  least_count = 16
  for ipa in used_ipa:
    for word in new_words:
      if ipa in word:
        ipa_count += 1
    if ipa_count < least_count:
      least_count = ipa_count
      least_ipa = ipa
    ipa_count = 0
  for word in new_words:
    if least_ipa in word:
      least_word = word
      break
  new_library.extend(new_words)
  return least_word
least_word = generate_loop("kaɾazabɯ\n")
for i in range(100):
  generate_loop(least_word)

print(new_library)
print(len(new_library))

with open("new_library.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(new_library))